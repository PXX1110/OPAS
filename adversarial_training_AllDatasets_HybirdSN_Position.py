...
fh = logging.FileHandler("result/tests_tf_" + str(testFile) + "_One_1_HSN_position_" + str(dataset) + ".log")
...
# Reshape data into (B, 1, C, H, W) for model
...
keras_callbacks = [ModelCheckpoint(filepath="checkpoint/org/checkpoint_org_" + str(testFile) + "_One_1_HSN_position_"
                  ...
                  CSVLogger("result/org/" + str(testFile) + "/logs_org_" + str(testFile) + "_One_1_HSN_position_" + 
                  str(dataset) + ".csv", separator=';', append=True)]
adam = adam_v2.Adam(lr=0.001, decay=1e-06)                             
model.compile(loss='categorical_crossentropy', optimizer=adam, metrics=['accuracy'])

## load the model architecture and weights
# model = load_model("checkpoint/org/checkpoint_org_" + str(testFile) + "_One_1_HSN_position_" + str(dataset) + ".hdf5")
...
plt.savefig("./result/org/" + str(testFile) + "/model_accuracy_org_" + str(testFile) + "_One_1_HSN_position_" + str(dataset) + ".png")
# plt.show()
plt.close()  # 关闭窗口
...
plt.grid(True)
plt.legend(['train', 'test'], loc='upper left') 
plt.savefig("./result/org/" + str(testFile) + "/model_loss_org_" + str(testFile) + "_One_1_HSN_position_" + str(dataset) + ".png")
# plt.show()
plt.close()  # 关闭窗口

## Evaluate the classifier on the test samples
classification, confusion, Test_loss, Test_accuracy, oa, each_acc, aa, kappa = reports(X_test,y_test,model)
...
plot_confusion_matrix(confusion, normalize=False, title='Confusion matrix')
plt.savefig("./result/org/" + str(testFile) + "/confusion_matrix_org_" + str(testFile) + "_One_1_HSN_position_"
            + str(dataset) + ".png")
# plt.show()
plt.close()
outputs = calculate_image(model, dataset=dataset)
spectral.save_rgb("test_result/org/" + str(testFile) + "/predictions_org_" + str(testFile) + "_One_1_HSN_position_"
                 + str(dataset) + ".png", outputs, colors=spectral.spy_colors)
...
outputs = calculate_image(model, dataset=dataset, adv=True, testFile=testFile)
...
# plt.show()
plt.close()
outputs = calculate_image(advmodel, dataset=dataset)
