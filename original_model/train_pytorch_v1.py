...
print("Create PixelAttack attack")
...
# preds = np.argmax(classifier.predict(Xtest_adv), axis=1)
...
advtrain_loader = torch.utils.data.DataLoader(dataset=advtrainset, batch_size=Batch_Size, shuffle=True, num_workers=0)
...
# 生成分类报告*/*
advclassification = classification_report(ytest, y_pred_test, digits=4)
print('Finished advtest')

# preds = np.argmax(classifier.predict(Xtest_adv), axis=1)
# acc = np.sum(preds == np.argmax(ytest, axis=1)) / ytest.shape[0]
# print("Classifier after adversarial training")
# print("Accuracy on adversarial samples: %.2f%%", (acc * 100))

print("classification:",classification,"advclassification:",
        advclassification,"advtraintime:",traintoc-traintic
        # ,testtoc-testtic
        )
